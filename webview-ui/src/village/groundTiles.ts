...
const P: Record<string, string> = {
  ...
  B: '#6B5035', K: '#5A4028', H: '#7D6345', P: '#64A050', Q: '#4B823C',
...
function selectBaseSprite(tileType: TileType, col: number, row: number): SpriteData {
  ...
  if (tileType === TileType.GARDEN) {
    const gardenColors = [P.B, P.B, P.B, P.H, P.P, P.Q]
    const base = garden_base.map((r) => [...r])
    let h = (col * 374761393 + row * 668265263) | 0
    for (let pr = 0; pr < 32; pr++) {
      for (let pc = 0; pc < 32; pc++) {
        if (!base[pr][pc]) continue
  ...
  if (tileType === TileType.WOODCUTTING) {
    const woodColors = [P.E, P.E, P.F, P.M]
    const base = wood_base.map((r) => [...r])
    let h = (col * 374761393 + row * 668265263) | 0
    for (let pr = 0; pr < 32; pr++) {
      for (let pc = 0; pc < 32; pc++) {
        if (!base[pr][pc]) continue
